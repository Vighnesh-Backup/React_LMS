...
const AllUsers = () => {
  ...
  let url = "http://localhost:8585/api/v1/getAllUsers";
  ...
  async function fetchdata(url) {
    ...
    setResponseResult(result.message);
...
if(ResponseResult.length > 0){
  return (
    <>
      ...
      <Navbar />
      <div className="flex flex-col justify-center p-16 "><h1 className=" text-5xl font-extrabold mb-4 text-gray-900 text-center">Some Of <h1 className=" inline-block text-5xl font-extrabold mb-4 text-transparent bg-clip-text bg-gradient-to-r to-emerald-600 from-sky-400"> OUR USER'S </h1></h1><h1 className=" text-center text-5xl font-extrabold mb-4 text-gray-900 ">Who are enjoying our Services</h1></div>
      
      <div className="flex  flex-wrap ml-16">
        {ResponseResult.map((item, index) => (

          <div className="flex flex-col justify-center max-w-xs m-8 p-6 shadow-md rounded-xl sm:px-12  dark:bg-gray-50 dark:text-gray-800">
            ...
            <div className="space-y-4 text-center divide-y dark:divide-gray-300">
              <div className="my-2 space-y-1">
                <h1 className="text-xl font-semibold sm:text-2xl" key={index}>{item.fname + " " + item.lname}</h1>
                <p className="px-5 text-xs sm:text-base dark:text-gray-600">{item.email}</p>
                <p className="px-5 text-xs sm:text-base dark:text-gray-600">{item.mobile}</p>
              </div>
  ...
  return(
	...
	<div className="container flex flex-col items-center justify-center px-5 mx-auto my-8 space-y-8 text-center sm:max-w-md">
		...
		<p className="text-3xl">Looks like Someting went Wrong!</p>
    <Link to = "/">
